...
/// Uses the session's model to generate a short, human-friendly title from the
/// conversation so far.
Future<String> generateSessionTitle({
  ...
  final transcript = _transcript(session);
  if (transcript.isEmpty) {
    throw StateError('会话内容为空，无法自动命名');
  }
...
String _transcript(Session session) {
  ...
  for (final message in session.messages) {
    ...
    buffer.writeln('${message.role == MessageRole.user ? '用户' : '助手'}：${_clip(text, 500)}');
